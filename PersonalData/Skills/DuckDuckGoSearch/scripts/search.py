...
def news_search(query: str, max_results: int = 5, fetch_content: bool = True) -> list:
    results = []
    with DDGS() as ddgs:
        for r in ddgs.news(query, max_results=max_results):
            ...
            if fetch_content and result["url"]:
                result["full_content"] = fetch_page_content(result["url"])
            results.append(result)
    return results
...
def search(query: str, search_type: str = "text", max_results: int = 5, fetch_content: bool = True) -> dict:
    ...
    try:
        if search_type == "text":
            ...
            results = news_search(query, max_results, fetch_content)
